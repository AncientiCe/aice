...
- **Classifier optimizations:** The system prompt and output schema are dynamically pruned to only enabled skills. Ollama JSON Schema structured output (`classifier_structured_output_enabled`) enables grammar-constrained decoding so the model cannot produce invalid intent strings. A context window cap (`classifier_num_ctx`) reduces VRAM use and prefill time. The 7b model is kept for classification to preserve reasoning (e.g. mapping "I'd like to eat some strawberries" to `skill_shopping_list`).
