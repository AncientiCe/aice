...
- Staff check guests and residents in and out under **Stays**. Memory exists only for an open stay whose guest or resident agreed (default: not agreed: ask each resident) and follows `memory_retention` at checkout ([section 26](../architecture/README.md#26-stays-and-stay-scoped-memory)). Default retention: `keep`.
