...
#[tokio::test]
async fn care_binary_escalates_a_fall_without_a_property_mcp() {
    let bin = match std::env::var("CARGO_BIN_EXE_aice-care") {
        Ok(path) => path,
        Err(error) => panic!("missing aice-care binary: {error}"),
    };
    let port = free_port();
    let (child, base, stderr_path) = spawn_pack(&bin, "care", port);
    wait_for_desk(&base, &stderr_path).await;
    let client = match reqwest::Client::builder().build() {
        Ok(client) => client,
        Err(error) => panic!("failed to build http client: {error}"),
    };
    let created: serde_json::Value = match client
    ...
    assert_eq!(
        created["result"]["structuredContent"]["status"],
        "escalated"
    );
    ...
    assert!(desk.contains("report_fall"));
    drop(child);
}
