...
async fn ward_binary_allows_wayfinding_and_denies_medication() {
    ...
    let pack = spawn_pack(&bin, "ward", json!({}), &[]);
    let token = wait_for_token(&pack).await;
    let (status, allowed) = call_tool(&pack.base, Some(&token), "wayfinding", "4B").await;
    assert_eq!(status, reqwest::StatusCode::OK);
    assert_eq!(allowed["result"]["structuredContent"]["status"], "open");
    let (_, denied) = call_tool(&pack.base, Some(&token), "prescribe_medication", "4B").await;
    ...
    assert!(desk.contains("aice-ward desk"));
    assert!(desk.contains("prescribe_medication"));
}
